...
resume = False
start  = 60
if not(run_cloudy): resume = True

flag = 0

def emmisivity(nH=1e-4, temperature=1e6, metallicity=1.0, redshift=0., indx=None, mode='PIE'):
    plotIt = False
    global flag
    ...
    if (mode!='CIE') and (mode!='PIE'):
        if (rank==0): print('Problem!', flush=True)
        comm.Disconnect()
        sys.exit(1)
...
save diffuse continuum "{"emission_%s_%09d.lin"%(mode,1000000+rank) if indx==None else "emission_%s_%09d.lin"%(mode,indx)}" units keV 
    """
    stream = stream[1:]
    ...
    else: filename = "autoGenScript_%s_%09d.in"%(mode,1000000+rank)
    
    with open("./auto/%s"%filename, "w") as text_file:
        text_file.write(stream)
    
    #process = subprocess.check_call(["./cloudy.exe", "-r %s"%filename[:-3]], shell=True, cwd='./auto/')
    #process.wait()
    ...
    data = np.loadtxt("./auto/emission_%s_%09d.lin"%(mode,1000000+rank) if indx==None else "./auto/emission_%s_%09d.lin"%(mode,indx) )
    if indx==None: 
...
if (rank==0):
    ...
    if (make_batch):
        print('Collecting cloudy outputs to .hdf5 batch files will also be performed.', flush=True)
        

if rank==0 and not(resume): os.system('rm -rf ./auto')
if (resume and rank==0): print("Resuming from stopped state ... ", flush=True)
if (not(run_cloudy) and rank==0): print("Not running cloudy. Assuming all text data already generated!", flush=True)
...
#start = start - extra  
if not(resume) or (start<0): start = 0
if rank==0: progbar.progress(min(start+rank, len(values)), len(values))
